...
class VoicePipeline:
    ...
    def process_audio(self, audio_bytes: bytes, session_id: str, target_lang: str = "auto") -> PipelineResult:
        ...
        # Use UI language toggle to guide STT — if user selected Hindi, transcribe in Hindi
        # If "auto", let Whisper auto-detect the language naturally
        stt_lang = None if target_lang == "auto" else target_lang
        stt_result = self.stt.transcribe(audio_bytes, force_language=stt_lang or "auto")
        ...
        # Use target_lang for LLM output; if auto, use what Whisper detected
        lang = stt_result.language if target_lang == "auto" else target_lang
        # Normalize to en/hi only
